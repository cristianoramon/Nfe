...
function b_mDown_ns4(nr) {
  ...
  l.write("' border=0></A></SPAN><Font size=2 Face=Arial color=black>");
  l.write(this.lbl[nr]+"</FONT><BR><BR>");
  l.close();
  this.sta[nr]=1;
}
...
function b_writeStyle() {
  ...
  document.write(' border-style:outset; border-color:#EFEFE7; ');
  document.write('text-color:white;background-image:url(Figuras2/botao2.jpg);}');
  
  //CSC Background
  document.write('.backFundo { background-image:url(Figuras2/Fundo2.jpg);}');
  ...
  if (bt.ns4) {                 //draw OutlookBar for ns4
    ...
    for (i=0;i<this.panels.length;i++) {
      document.write('<Layer name='+this.name+'_panel'+i+' width='+this.width);
       ...
       document.write(this.width+',28" bgcolor=#EFEFE7 class=button ');
    ...
    for (i=0;i<this.panels.length;i++) {
      ...
      document.write(this.width+'px, '+h+'px, 0px); background-image:url(Figuras2/fundo10.jpg);">');
