...
$selPedido = " select   p.val_total,ip.qtd_pedida,".
              ...
              " and p.pedido ='$in_var'";
$sql_statement = OCIParse($conn, $selPedido) or die ("Falha na passagem de cláusula SQL selPedido.");
 ...
 //Sequencia
 $sel = "SELECT SEQ_NFE.NEXTVAL seq FROM DUAL ";
 //$sql_seq = OCIParse($conn, $sel) or die ("Falha na passagem de cláusula SQL selPedido.");
 //OCIExecute($sql_seq) or die ("Não foi possível executar a cláusula SQL selPedido.");
 //Ocifetch($sql_seq );
 
 //$seq = ociresult($sql_statement,"SEQ");
...
*/
 ...
 for ( $qtPro = 0 ; $qtPro < count($codProd) ; $qtPro++){
  ...
  //echo "<br>Qtde2 " . $qtPro ."-".$codProd[$qtPro];
   ...
   $inSert = "INSERT INTO T_TMP_NFE_SAIDA(  SERIE,
  											...
  											CODDEPOSITO,
  											PEDIDO)
					VALUES ('$serie','$filial','".$codProd[$qtPro]."','".$qtdeProd[$qtPro]."','".$codDepo[$qtPro]."','$in_var')";
 ...
 $s = OCIParse($conn, "begin igravanfsaidav3(:pedido, :filial, :NfMae, :pl ,".
                      ":pb,:codTransportadora,:snf,:serie,:obs,:cfop); end;");
...
// echo "window.open(\"https://www.crpaaa.com.br:8443/GerarNota2/$nomNota\");";
 //echo "location.href=\"https://www.crpaaa.com.br:8443/GerarNota2/NotaGerada.php?nmNota=$nomNota&xml=$xml\";";
